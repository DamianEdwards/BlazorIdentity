...
internal class BlazorServerUserManager<TUser> : IBlazorUserManager<TUser> where TUser : class
{
    ...
    {
        var results = await _userManager.GetLoginsAsync(user);
    ...
    public async Task<IdentityResult> CreateAsync(TUser user)
        => ToBlazorIdentityResult(await _userManager.CreateAsync(user));
    ...
    public async Task<bool> IsEmailConfirmed(TUser user) => await _userManager.IsEmailConfirmedAsync(user);
